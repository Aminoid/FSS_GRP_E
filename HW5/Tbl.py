...
class Tbl:
    ...
    def categories(self, txt):
        spec = [
            when_what_weight_where("$",    Num.c(),     1,   [self.all.cols, self.x.cols, self.all.nums, self.x.nums]),
            when_what_weight_where("%",    Num.c(),     1,   [self.all.cols, self.x.cols, self.all.nums, self.x.nums]),
            when_what_weight_where("<",     Num.c(),    -1,  [self.all.cols, self.y.cols, self.all.nums, self.goals, self.less, self.y.nums]),
            when_what_weight_where(">",     Num.c(),     1,  [self.all.cols, self.y.cols, self.all.nums, self.goals, self.more, self.y.nums]),
            when_what_weight_where("!",     Sym.s(),      1,  [self.all.cols, self.y.syms, self.y.cols,   self.all.syms]),
            when_what_weight_where("",      Sym.s(),      1,  [self.all.cols, self.x.cols, self.all.syms, self.x.syms])
    ...
    def consume(self, result):
        ...
        for key in result.keys():
            ...
            if isinstance(a, Sym.s):
                s= Sym.s(p)
                s.updates(result.get(key))
                # s.pos(p)
                self.all.syms[p] = s
                self.all.cols[p] = s
            elif isinstance(a, Num.c):
                n = Num.c(p)
                # n.pos(p)
